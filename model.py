...
class Model():
    def __init__(self, seasonal=True, params=None, no_dead=False, const_u1=None):
        ...
        if const_u1 is None or const_u1 == 0:    # NOTE: this is called in self.f when u1 = 0 and T = 0 at a specific timestep
...
def sy_f_simple(reintroduction=False):
    ...
    if reintroduction:
        ...
        dS2 = .27 * nu + phi * S1 - (alpha * I + eps) * S2 - d2 * S2 - zeta * S2 * (I - (T / (K - zeta * V))) + gamma * T  # TODO: figure out a good way to incorporate vaccinated frogs going into frog saunas
        ...
        dV = .73 * nu - d2 * V - zeta * V * (1 - (T / K)) + gamma * T * (V / (S2 + I + V))                                     # is the current setup a good proxy?
